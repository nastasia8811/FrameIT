...
const Filter: FC<FilterProps> = ({ query, setQuery, minRating, setMinRating, sort, setSort }) => {
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            <div className="flex items-center gap-3">
                <label className="text-sm opacity-75" style={{ color: colors.primary }} >Min rating</label>
                <input
                ...
                />
                <span className="text-sm tabular-nums w-10 text-right"  style={{ color: colors.primary }}>{minRating.toFixed(1)}</span>
            </div>

            <div>
                <label className="sr-only"  style={{ color: colors.primary }}>Sort</label>
                <select  style={{ color: colors.primary }}
                    value={sort}
